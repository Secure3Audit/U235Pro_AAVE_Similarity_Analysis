...
const func: DeployFunction = async function ({
  ...
  const poolConfig: IAaveConfiguration = await loadPoolConfig(
    MARKET_NAME as ConfigNames
  );

  if (poolConfig.DisableIncentives === true) {
    console.log(
      `[Deployment] Skipping Incentives deployment for market: ${MARKET_NAME}`
    );
    return;
  }
  ...
  const isInit = await getInitConfiguration(incentivesImpl.address);
  if (!isInit) {
    ...
    await saveInitConfiguration(incentivesImpl.address);
  }
