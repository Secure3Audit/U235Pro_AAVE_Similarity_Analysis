...
const func: DeployFunction = async function ({
  ...
  const isInit = await getInitConfiguration(treasuryImpl.address);
  if (!isInit) {
    ...
    await saveInitConfiguration(treasuryImpl.address);
  }
  ...
  const isTreasuryInit = await getInitConfiguration(
    treasuryImplArtifact.address
  );
  if (!isTreasuryInit) {
    await waitForTx(
