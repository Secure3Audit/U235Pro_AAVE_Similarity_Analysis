...
export const getInitConfiguration = async (
  address: tEthereumAddress
): Promise<boolean> => {
  try {
    const configuration = await hre.deployments.readDotFile(
      INIT_CONFIGURATION_ID
    );
    const configurationJson = JSON.parse(configuration);
    return configurationJson[address];
  } catch (e) {
    return false;
  }
};

export const saveInitConfiguration = async (address: tEthereumAddress) => {
  let configurationJson;
  try {
    const configuration = await hre.deployments.readDotFile(
      INIT_CONFIGURATION_ID
    );
    configurationJson = JSON.parse(configuration);
  } catch (e) {
    configurationJson = {};
  }

  configurationJson[address] = true;
